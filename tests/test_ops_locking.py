...
def test_collect_waits_for_operation_lock_then_writes_snapshot(repo_root: Path, write_config, tmp_path: Path) -> None:
    root = tmp_path / "root"
    create_sample_tree(root)
    config_path = write_config(roots=[root], included_filesystems=["tmpfs"])
    db_path = tmp_path / "watchdirs.sqlite3"
    ...
    payload = parse_json_output(subprocess.CompletedProcess(process.args, process.returncode, stdout, stderr))
    assert payload["ok"] is True
    assert payload["snapshots"][0]["status"] == "complete"
    assert _snapshot_count(db_path) == 1


def test_collect_lock_timeout_exits_before_database_creation(repo_root: Path, write_config, tmp_path: Path) -> None:
    root = tmp_path / "root"
    create_sample_tree(root)
    config_path = write_config(roots=[root], included_filesystems=["tmpfs"])
    db_path = tmp_path / "watchdirs.sqlite3"
    ...
    assert payload["ok"] is False
    error = payload["error"]
    assert error["code"] == "operation_lock_timeout"
    assert error["db_path"] == str(db_path)
    assert error["lock_path"] == str(lock_path)
    assert error["lock_timeout_seconds"] == 0.1
